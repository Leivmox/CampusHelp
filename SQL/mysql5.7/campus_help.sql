/*
 ...
 Date: 18/10/2025 20:59:43
*/
-- 创建数据库（如果不存在）
CREATE DATABASE IF NOT EXISTS `campus_help` DEFAULT CHARACTER SET utf8mb3 COLLATE utf8mb3_general_ci;
...
CREATE TABLE `class`  (
  ...
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `FK_CLASS_SCHOOL`(`school_id` ASC) USING BTREE,
  INDEX `FK_CLASS_DEPT`(`dept_id` ASC) USING BTREE,
...
CREATE TABLE `dept`  (
  ...
  `school_id` int NOT NULL COMMENT '学校id',
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `FK_DEPT_SCHOOL`(`school_id` ASC) USING BTREE,
  INDEX `name`(`name` ASC) USING BTREE,
...
CREATE TABLE `sxc_remark`  (
  ...
  INDEX `accept_id`(`accept_id` ASC) USING BTREE,
...
CREATE TABLE `task`  (
  ...
  `state` int NOT NULL DEFAULT 0 COMMENT '状态',
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `FK_TASK_PUBLISH_USER`(`publish_user_id` ASC) USING BTREE,
  INDEX `FK_TASK_ACCEPT_USER`(`accept_user_id` ASC) USING BTREE,
  INDEX `FK_TASK_USER_SCHOOL`(`user_school_id` ASC) USING BTREE,
...
CREATE TABLE `user`  (
  ...
  `state` int NOT NULL DEFAULT 0 COMMENT '状态',
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `FK_USER_SCHOOL`(`school_id` ASC) USING BTREE,
  INDEX `FK_USER_DEPT`(`dept_id` ASC) USING BTREE,
  INDEX `FK_USER_CLASS`(`class_id` ASC) USING BTREE,
