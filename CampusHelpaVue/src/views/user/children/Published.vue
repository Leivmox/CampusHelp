...
<!--                        <i class="el-icon-edit" style="cursor: pointer; color: #66b1ff" v-show="item.state == 0"/>-->
                        <el-popconfirm title="确定取消求助吗？" @confirm="cancel(item.id)" v-show="item.state == 0">
                            <el-button style="float: right; padding: 3px 0" type="text" slot="reference">取消求助
                            </el-button>
                        </el-popconfirm>
                    </template>
                </div>
                ...
                <el-button type="primary" style="float: right;margin:10px 0;" @click="completeTask(item.id)"
                           v-show="item.state==1">完成求助
                </el-button>

            </el-card>
            <div style="text-align: center" v-if="tasks.length == 0">
                <span><i class="el-icon-refresh-right"></i>暂无发布求助</span>
            </div>
        </el-card>
