...
public class ClassController {
    ...
    // 添加myClass
    @PostMapping
    public Map<String, Object> saveClass(@RequestBody Class c) {
        // 验证school_id是否存在
        if (schoolService.getById(c.getSchoolId()) == null) {
            return message.message(false, "error, 学校不存在", "", null);
        }

        // 验证dept_id是否存在，并且属于同一个school_id
        Dept dept = deptService.getById(c.getDeptId());
        if (dept == null) {
            return message.message(false, "error, 系别不存在", "", null);
        }
